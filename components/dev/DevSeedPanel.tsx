    ...
    <Card className="border-amber-300/80 bg-amber-50/40 dark:border-amber-600/50 dark:bg-amber-950/30">
      <div className="flex flex-col gap-3 p-5 sm:flex-row sm:items-start sm:justify-between sm:p-6">
        <div className="flex gap-3">
          ...
          <div>
            ...
            <CardDescription className="mt-1 text-sm text-amber-900/90 dark:text-amber-100/85">
              Inserts three test outlets and reviews (IDs prefixed with <code className="rounded bg-black/5 px-1 dark:bg-white/10">rp-seed-</code>).
              Re-running replaces only that sample data. Requires <code className="rounded px-1">ALLOW_DEV_SEED=true</code> in{' '}
        ...
        <Button
          ...
          disabled={loading}
          onClick={() => void run()}
          className="shrink-0 rounded-xl border-amber-400/80 bg-white/90 dark:border-amber-600 dark:bg-slate-900/60"
        >
          {loading ? (
            <>
