...
/** Send a one-off test message to the user’s saved WhatsApp number (any plan — verifies Twilio + sandbox). */
export async function POST() {
  try {
    ...
    if (!num) {
      return err('Save your WhatsApp number first (we store E.164, e.g. +919876543210).', 400)
    }
